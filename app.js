...
const MAX_QUEUE_REFERENCE_THUMBS = 5;
const INSPIRATION_PROMPTS = [
  "帮我做一篇适合小红书发布的春夏通勤穿搭笔记，面向 25-35 岁上班族，语气自然真实，有轻松的分享感，突出显瘦、舒适、好搭配。",
  "帮我生成一篇周末城市漫游图文，风格像朋友随手记录的一天，文案要有画面感，适合配咖啡、街景、穿搭和生活碎片。",
  "帮我写一篇新品种草笔记，目标用户是正在做购买决策的人，重点突出使用场景、细节质感和真实体验，避免夸张营销语。",
  "帮我做一篇旅行出片攻略，内容包含拍照氛围、路线节奏和穿搭建议，语气轻松但信息密度高，适合收藏和转发。",
  "帮我生成一篇生活方式分享笔记，主题是把普通日常过得更有仪式感，文案温柔克制，带一点治愈感和实用建议。",
];
...
function bindEvents() {
  ...
  els.inspirationBtn.addEventListener("click", applyInspirationPrompt);
...
function applyInspirationPrompt() {
  const nextIndex = getNextInspirationIndex();
  appState.inspirationIndex = nextIndex;
  els.extraPrompt.value = INSPIRATION_PROMPTS[nextIndex];
  syncCounts();
  persistDraft();
  els.extraPrompt.focus();
}

function getNextInspirationIndex() {
  if (INSPIRATION_PROMPTS.length === 1 || appState.inspirationIndex === null) {
    return Math.floor(Math.random() * INSPIRATION_PROMPTS.length);
  }
  const offset = Math.floor(Math.random() * (INSPIRATION_PROMPTS.length - 1)) + 1;
  return (appState.inspirationIndex + offset) % INSPIRATION_PROMPTS.length;
}
...
function renderQueue() {
  ...
  els.queueBody.innerHTML = appState.tasks
    .map((task) => {
      ...
      const previewThumbs = referenceImages.slice(0, MAX_QUEUE_REFERENCE_THUMBS);
      const hiddenCount = referenceImages.length - previewThumbs.length;
      return `
        <tr>
          <td>
            <div class="queue-materials">
              ...
              ${
                hiddenCount > 0
                  ? `<div class="queue-plus">+${hiddenCount}</div>`
                  : ""
              }
            </div>
          </td>
